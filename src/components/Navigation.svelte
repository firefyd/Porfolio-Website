...
<div class="flex items-center md:hidden">
	<button
		on:click={toggleMenu}
		class="p-2 ml-2 rounded-md text-text-secondary hover:text-text-main hover:bg-bg-secondary transition-colors focus-visible-ring"
...
<ul class="hidden md:flex items-center gap-8">
	{#each navLinks as link}
		<li>
			<a href={link.href} class="text-text-secondary hover:text-text-main transition font-medium focus-visible-ring">
				{link.name}
			</a>
		</li>
	{/each}
			...
			{#each navLinks as link}
				<li>
					<a
						href={link.href}
						on:click={toggleMenu}
						class="block py-2 text-xl text-text-main hover:text-accent-hover transition focus-visible-ring"
					>
						{link.name}
					</a>
				</li>
