...
<button 
    ...
    class="md:hidden p-2 rounded-lg text-text-main hover:text-accent transition-colors duration-200 focus-visible-ring z-[60] relative h-10 w-10 flex items-center justify-center"
        ...
        <nav class="flex flex-col pt-24 space-y-2 px-6">
            {#each navLinks as link}
                <a 
                    ...
                    class="text-2xl py-3 px-4 rounded-lg transition-colors duration-200 font-semibold w-full text-left focus-visible-ring"
                >
                    {link.name}
                </a>
            {/each}
